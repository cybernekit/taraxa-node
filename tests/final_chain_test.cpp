...
TEST_F(FinalChainTest, mining_rewards_distribution) {
  ...
  // Create pbft miner
  KeyPair pbft_miner = KeyPair::create();
  cfg.state.genesis_balances[pbft_miner.address()] = 0;

  // Create dag miners
  std::vector<KeyPair> dag_miners;
  dag_miners.reserve(MINERS_NUM);
  for (size_t i = 0; i < MINERS_NUM; ++i) {
    auto const& k = dag_miners.emplace_back(KeyPair::create());
  ...
  for (const auto& block : blocks) {
    for (const auto& tx : block.txs) {
      if (dag_stats.addTransaction(tx.sha3(), block.miner)) {
        continue;
      }
