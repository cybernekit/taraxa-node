...
Executor::Executor(addr_t node_addr, std::shared_ptr<DbStorage> db, std::shared_ptr<DagManager> dag_mgr,
  ...
  trxs_tmp_buf_.reserve(expected_max_trx_per_block);
  trxs_stats_tmp_buf_.reserve(expected_max_trx_per_block);
}
...
void Executor::execute_(PbftBlock const &pbft_block) {
  ...
  auto batch = db_->createWriteBatch();
  {
    ...
    DbStorage::MultiGetQuery db_query(db_, trxs_tmp_buf_.capacity() + 100);
    ...
    for (auto const &dag_blk_raw : dag_blks_raw) {
      ...
      // TODO: Check if dag block is validated to consist at least 1 trx. Is this checked before ???

      const addr_t &dag_block_author = dag_block.getSender();

      for (auto const &trx_hash : dag_block.getTrxs()) {
        if (dag_stats_tmp_buf_.addTransaction(trx_hash, dag_block_author)) {
          // Trx was already processed
          continue;
        }

        db_query.append(DbStorage::Columns::executed_transactions, trx_hash);
        db_query.append(DbStorage::Columns::transactions, trx_hash);
      }
    ...
    for (uint i = 0; i < trx_db_results.size() / 2; ++i) {
      size_t executed_trx_result_idx = 0 + i * 2;
      size_t trx_result_idx = 1 + i * 2;

      h256 trx_hash = h256(db_query.get_key(trx_result_idx));

      auto has_not_been_saved = trx_db_results[trx_result_idx].empty();
      if (has_not_been_saved) {
        // This should never happen
        throw std::runtime_error("Executor::executePbftBlocks_: invalid transaction is being processed: trx_hash: " +
                                 trx_hash.toString());
      }

      bool has_been_executed = !trx_db_results[executed_trx_result_idx].empty();
      ...
      // Non-executed trxs
      auto trx =
          dev::eth::Transaction(&trx_db_results[trx_result_idx], dev::eth::CheckTransaction::None, true, trx_hash);

      if (replay_protection_service_->is_nonce_stale(trx.sender(), trx.nonce())) {
        continue;
      }

      trxs_tmp_buf_.emplace_back(std::move(trx));
      trxs_stats_tmp_buf_.emplace_back(dag_stats_tmp_buf_.getTransactionStatsRvalue(trx_hash));
  ...
  auto const &[new_eth_header, trx_receipts, _] =
      final_chain_->advance(batch, pbft_block.getBeneficiary(), pbft_block.getTimestamp(), trxs_tmp_buf_,
                            trxs_stats_tmp_buf_, dag_stats_tmp_buf_.getBlocksStats());

  // Update replay protection service, like nonce watermark. Nonce watermark has been disabled
  replay_protection_service_->update(batch, pbft_period, util::make_range_view(trxs_tmp_buf_).map([](auto const &trx) {
  ...
  auto num_executed_trx = num_executed_trx_ + trxs_tmp_buf_.size();
  if (!finalized_dag_blk_hashes.empty()) {
    ...
    LOG(log_nf_) << node_addr_ << " :   Executed dag blocks index #"
                 ...
                 << " , Transactions count: " << trxs_tmp_buf_.size();
  }
  ...
  trx_mgr_->getPendingBlock()->advance(
      ...
      util::make_range_view(trxs_tmp_buf_).map([](auto const &trx) { return trx.sha3(); }));
